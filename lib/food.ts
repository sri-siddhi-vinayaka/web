...
// Goes through the claimed_dishes() RPC (see
// infra/migrations/0003_food_registrations.sql) rather than
...
// DEFINER function that returns only the dish names.
export async function getClaimedDishes(): Promise<string[]> {
  if (!isSupabaseConfigured) return [];

  try {
    const { data, error } = await withTimeout(
      supabase.rpc("claimed_dishes") as unknown as Promise<{
