...
import InlineYouTubeToggle from "@/components/InlineYouTubeToggle";
import { CRICKET_TOURNAMENTS, toYouTubeEmbedUrl } from "@/lib/config";
            ...
            <ul className="mt-3 flex flex-col gap-2">
              {tournament.videos.map((video) => {
                ...
                return (
                  <li key={video.url}>
                    {embedUrl ? (
                      <InlineYouTubeToggle label={video.label} embedUrl={embedUrl} />
                    ) : (
                      <a
                        href={video.url}
                        target="_blank"
